...
def compute_maba(model, train_loader, test_loader, device):
    ...
    with torch.no_grad():
        for images, labels, bias1, bias2 in train_loader:
            all_train_labels.extend(labels.cpu().numpy())
            all_train_bias1.extend(bias1.cpu().numpy())
            all_train_bias2.extend(bias2.cpu().numpy())
        
        for images, _, bias1, bias2 in test_loader:
    ...
    delta_gm = {}
    print("\nGroup | Bias1 | Bias2 | Train Bias (Num/Denom) | Test Bias (Num/Denom) | Delta_gm")
    print("-" * 90)
    ...
    for key in bias_train:
        ...
        # Apply log and then tanh to train and test biases
        train_bias = (train_num / train_denom)*100 if train_denom != 0 and train_num > 0 else 0
        test_bias = (test_num / test_denom)*100 if test_denom != 0 and test_num > 0 else 0
        
        # Enforce the condition for Delta_gm
        if train_bias > np.tanh(np.log(1 / len(np.unique(all_train_labels)))):
    ...
    # Create bias data table
    bias_data = []
    for key, value in bias_test.items():
        bias_data.append([key[0], key[1][0] if key[1][0] is not None else 'N/A',
                           key[1][1] if key[1][1] is not None else 'N/A', value])

    bias_df = pd.DataFrame(bias_data, columns=["Target", "Bias1", "Bias2", "Count"])
    print("\nBias Table:")
    print(bias_df)
