...
def train(train_loader, model, criterion, optimizer, protected_net, protected_net2, opt):
    ...
    for idx, (images, labels, biases, biases2, _) in enumerate(train_iter):
        ...
        shuffled_pr_feat2 = pr_feat2[-1].clone()  
        ...
        logits, features = model.concat_forward(images, shuffled_pr_feat, shuffled_pr_feat2)
...
def fine_tune(train_loader, val_loader, model, criterion, optimizer, protected_net, protected_net2, opt, epochs=30):
    model.train()
    protected_net.eval()
    protected_net2.eval()
    decay_epochs = []
    ...
    best_acc = 0.0
    best_model_path = f"RESULTS-ablation-grad-supp-negrishi/{opt.exp_name}/checkpoints/fine_tuned_model_grad_sup.pth"
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=len(train_loader), desc=f"Epoch {epoch+1}/{epochs}") as pbar:
            for idx, (images, labels, biases, biases2, _) in enumerate(train_iter):
                ...
                pr1_proj = project(model_feat, pr_feat)
                pr2_proj = project(model_feat, pr_feat2)

                # Compute residuals
                pr1_residual = pr1_proj - model_feat
                pr2_residual = pr2_proj - model_feat
...
def main():
    ...
    output_dir = f"RESULTS-ablation-grad-supp-negrishi/{exp_name}"
    ...
    beta_values = [0,0.01,1]  

    # Iterate over different alpha and beta values
    for alpha in alpha_values:
        for beta in beta_values:
            ...
            # Set new optimizer for fine-tuning
            optimizer = optim.Adam(model.parameters(), lr=opt.lr * 0.1)

            # Update opt with new alpha and beta
            opt.alpha = alpha
            opt.beta = beta
